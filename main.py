...
@app.get("/api/inventory/bulk-import-template")
def api_inventory_bulk_template(current_user=Depends(auth.admin_or_warehouse)):
    """Excel orqali, ko'p material qo'shish uchun — to'ldirilishi kerak bo'lgan,
    NAMUNA faylni yaratib, yuklab beradi."""
    import openpyxl
    ...
    headers = ["Nomi", "Birlik", "Boshlang'ich miqdor", "1 birlik narxi", "Minimal zaxira"]
    header_fill = PatternFill(start_color="B9783F", end_color="B9783F", fill_type="solid")
    for col, h in enumerate(headers, start=1):
        cell = ws.cell(row=1, column=col, value=h)
        cell.font = Font(bold=True, color="FFFFFF")
        cell.fill = header_fill

    # Namuna qatorlar — foydalanuvchi, buni o'chirib, o'z ma'lumotini yozadi
    sample_rows = [
        ["Bazalt to'ri 5mm", "dona", 100, 45000, 10],
        ["Kley quruq aralashma", "kg", 500, 8000, 50],
    ]
    for r_idx, row in enumerate(sample_rows, start=2):
        for c_idx, val in enumerate(row, start=1):
            ws.cell(row=r_idx, column=c_idx, value=val)

    for col, width in zip("ABCDE", [28, 12, 20, 18, 16]):
    ...
    return Response(
        ...
        headers={"Content-Disposition": "attachment; filename=xomashyo_namuna.xlsx"}
    )


@app.post("/api/inventory/bulk-import")
def api_inventory_bulk_import(file: UploadFile = File(...), db: Session = Depends(get_db),
                               current_user=Depends(auth.admin_or_warehouse)):
    """Excel fayldan, bir vaqtda, ko'plab xomashyoni, xavfsiz qo'shadi.
    MUHIM: allaqachon MAVJUD nomdagi material — TASODIFAN ustidan
    yozilmaydi, xavfsiz ravishda, "o'tkazib yuborilgan" deb belgilanadi."""
    import openpyxl

    if not file.filename.endswith(('.xlsx', '.xlsm')):
        raise HTTPException(status_code=400, detail="Faqat .xlsx formatidagi fayl qabul qilinadi")
    ...
    # 1-qator — sarlavha, shuning uchun, 2-qatordan boshlaymiz
    for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        name = row[0] if len(row) > 0 else None
        if not name or not str(name).strip():
            continue  # bo'sh qator — o'tkazib yuboramiz, xato emas

        name = str(name).strip()
        unit = str(row[1]).strip() if len(row) > 1 and row[1] else None
        stock_qty = float(row[2]) if len(row) > 2 and row[2] not in (None, "") else 0.0
        price = float(row[3]) if len(row) > 3 and row[3] not in (None, "") else None
        min_stock = float(row[4]) if len(row) > 4 and row[4] not in (None, "") else 0.0

        if not unit:
            errors.append({"qator": row_idx, "nomi": name, "sabab": "Birlik (masalan: kg, dona) ko'rsatilmagan"})
            continue

        existing = db.query(Inventory).filter(Inventory.item_name == name).first()
        if existing:
            skipped.append({"qator": row_idx, "nomi": name, "sabab": "Bu nomdagi material, omborda allaqachon mavjud"})
            continue

        try:
            item_data = schemas.InventoryCreate(
                item_name=name, unit=unit, stock_quantity=stock_qty,
                price_per_unit=price, min_stock=min_stock
            )
            crud.add_item(db, item_data)
            added.append(name)
        except Exception as e:
            db.rollback()
            errors.append({"qator": row_idx, "nomi": name, "sabab": str(e)})

    return {
        "success": True,
        "qoshildi_soni": len(added),
        "qoshilganlar": added,
        "otkazib_yuborilgan_soni": len(skipped),
        "otkazib_yuborilganlar": skipped,
        "xato_soni": len(errors),
        "xatolar": errors,
    }
