...
def generate_finished_sale_pdf(sale, db=None) -> bytes:
    ...
    info_rows = [
        ["Sana:", date_str, "To'lov usuli:", pay_labels.get(sale.payment_method, sale.payment_method or "—")],
        ["Xaridor:", sale.buyer_name or "—", "Sotuvchi:", sale.created_by or "—"],
    ]
    ...
    el.append(Paragraph("<b>Sotilgan mahsulot</b>", st_norm))
    ...
    data.append(["", "", "", "JAMI:", _fmt(sale.total_amount)])
...
def generate_delivery_pdf(delivery, db=None) -> bytes:
    ...
    for i, di in enumerate(delivery.items, 1):
        ...
        # MUHIM: ba'zi turlarda (masalan Termopanel/Bazalt) total_price
        # eski buyurtmalarda to'liq hisoblanmagan bo'lishi mumkin — bunday
        # holatda unit_price'ning o'zidan (agar bor bo'lsa) foydalanamiz,
        # shunda narx "0" bo'lib ko'rinib qolmaydi.
        total_price = float(oi.total_price or 0)
        if total_price > 0 and ordered > 0:
            unit_p = total_price / ordered
        else:
            unit_p = float(oi.unit_price or 0)
        line_sum = unit_p * qty
    ...
    tbl = Table(
        data,
        colWidths=[0.8*cm, 5.7*cm, 2.4*cm, 2.6*cm, 2.8*cm, 2.4*cm, 1.5*cm],
        repeatRows=1
    )
    style = [
        ('BACKGROUND', (0, 0), (-1, 0), DARK),
